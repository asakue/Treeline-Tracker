...
### Тестирование
* **Vitest + React Testing Library:** Покрытие модульными и интеграционными тестами слоев `shared/lib`, `entities` и ключевых `features`.
* **Typecheck & Lint:** Обязательная проверка типов `tsc --noEmit` и линтинг ESLint перед каждым коммитом.
